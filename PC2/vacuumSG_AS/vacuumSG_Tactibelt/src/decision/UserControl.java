...
public class UserControl extends Decision{
	...
	public double[] decision(float[] enacted){
		...
		float max=0;
		for (int v=0;v<NB_VIBRATOR;v++){
			if (direction_target[v]>0){
			//if (vibrators[v]>max){
				angle_max=v;
				max=direction_target[v];
			}
		}
		
		angle_max=(angle_max-70+360)%360;
		max=Math.max(0, Math.min(3, (max-10)/3)+6);
		
		//System.out.println("angle_max : "+angle_max+" ; max : "+max);
		if (started)
		if ( (angle_previous!=angle_max || pwm_previous!=max) && angle_max!=-1){
			System.out.println("angle_max : "+angle_max+" ; max : "+max);
			agent.communication.sendMsg(angle_max, (int)max, 0);
			angle_previous=angle_max;
			pwm_previous=(int) max;
		}
		
		//if (started) agent.communication.sendMsg(vibrators, direction_target);
